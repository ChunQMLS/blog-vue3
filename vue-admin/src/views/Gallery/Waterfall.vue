...
import { WaterfallData, WaterfallList } from '@/types/gallery'
...
const newData = computed(() => {
    return data.value.map( item => {
        item.height = Math.ceil(itemWidth/item.width*item.height)
...
const render = () => {
    const column = Math.floor(containerWidth.value/(itemWidth+gap.value))
    const arr:WaterfallList[] = new Array(column).fill({}).map( () => ({height: 0,waterfallDatas: []}))
    let minIndex = 0
...
<template>
    <div class="img-wall" ref="imgWall">
        <div class="img-list" v-for="(item, index) in list" 
            :key="index">
            <slot name="waterfill" :data="item.waterfallDatas" :gap="gap"></slot>
        </div>
    </div>
</template>
